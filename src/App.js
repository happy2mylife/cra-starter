...
function onCreate() {
  DataStore.save(
    new Todo({
      ...
      kinokotion: `kinokotion ${Math.random().toString(36).substring(2, 5)}`
    })
  );
}
  ...
  return (
    <div className="App">
      <header className="App-header">
        ...
        <table border="1">
          <thead>
            <tr><td>Id</td><td>Desc</td><td>Kinoko</td></tr>
          </thead>
          <tbody>
            {todos.map((item, i) => {
              return <tr key={i}><td>{todos[i].id.substring(0, 8)}...</td><td>{todos[i].description}</td><td>{todos[i].kinokotion}</td></tr>
            })}
          </tbody>
        </table>
      </header>
